    ...
    <section id='contact' className='max-w-[800px] text-center m-[0px_auto_100px] p-[100px_0px]'>
        ...
        <form onSubmit={handleSend}>
            <div className='my-4 text-center flex flex-col justify-center items-center'>
                ...
                <Input disabled={isLoading} name='name' onChange={(e) => setFormData((oldState) => ({...oldState, [e.target.name]: e.target.value}))} className='outline-0 border-green max-w-[90%] md:max-w-[50%] text-green py-6 px-4 placeholder:text-white/40' placeholder='Name' />
            </div>

            <div className='my-4 text-center flex flex-col justify-center items-center'>
                <span className='text-white'>Email:</span>
                <Input disabled={isLoading} name='email' onChange={(e) => setFormData((oldState) => ({...oldState, [e.target.name]: e.target.value}))} className='outline-0 border-green max-w-[90%] md:max-w-[50%] text-green py-6 px-4 placeholder:text-white/40' placeholder='Email' />
            </div>

            <div className='mt-6 mb-4 text-center flex flex-col justify-center items-center'>
                <span className='text-white'>Message:</span>
                <textarea disabled={isLoading} name='message' onChange={(e) => setFormData((oldState) => ({...oldState, [e.target.name]: e.target.value}))} placeholder='Write your message here...' className='outline-0 border-1 border-green max-w-[90%] md:max-w-[50%] text-green w-full rounded-xl p-[1rem_6rem_6rem_1rem]' />
            </div>


            <div className='hover:scale-115 mt-8 w-full flex justify-center'>
                <Button disabled={isLoading} className='w-full max-w-[50%] md:max-w-[20%] bg-transparent hover:bg-transparent cursor-pointer text-green border border-green rounded-md p-[1.5rem_6rem] leading-[1.6] cubicTransition duration-1000 text-[19px] resumeAnimation'>Send <Send /></Button>
            </div>
